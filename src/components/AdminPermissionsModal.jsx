...
import { Shield, Lock, Key, CheckCircle2, AlertCircle, X, Eye, EyeOff, Save, Copy, Check, User, Building, UserCheck } from 'lucide-react';
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        adminName: parsed.adminName || 'C.Rithvik',
        adminDept: (parsed.adminDept === 'CSE' ? 'CR' : (parsed.adminDept || 'CR')),
        adminRole: parsed.adminRole || 'ADMIN',
        requireAccessCode: parsed.requireAccessCode ?? false,
        crAccessCode: parsed.crAccessCode || 'VNR2026',
        allowEdits: parsed.allowEdits ?? true,
        allowDeletes: parsed.allowDeletes ?? false
      };
    }
  } catch (e) {}
  return {
...
export default function AdminPermissionsModal({ isOpen, onClose }) {
  ...
  // Editable Profile & Credentials state
  ...
  const [adminDept, setAdminDept] = useState(settings.adminDept);
  const [newAdminPass, setNewAdminPass] = useState(settings.adminPassword);

  // Permissions & Access Code state
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSettings(current);
      setAdminName(current.adminName);
      setAdminUsername(current.adminUsername);
      setAdminDept(current.adminDept);
      setNewAdminPass(current.adminPassword);
  ...
  const handleAdminLogin = (e) => {
    ...
    if (enteredPassword === settings.adminPassword || enteredPassword === 'admin') {
      ...
      setLoginError('Incorrect Admin password. (Default: admin)');
    }
  };
  ...
  const handleSaveSettings = () => {
    const updated = {
      ...settings,
      adminName: adminName.trim() || 'C.Rithvik',
      adminUsername: adminUsername.trim() || 'admin',
      adminDept: adminDept.trim() || 'CSE',
      adminRole: 'ADMIN',
      adminPassword: newAdminPass.trim() || 'admin',
    ...
    // Update active session storage user immediately
    const currentUser = sessionStorage.getItem('vnr_user');
    if (currentUser) {
      const parsed = JSON.parse(currentUser);
      const updatedUser = {
        ...parsed,
        displayName: updated.adminName,
        username: updated.adminUsername,
        dept: updated.adminDept,
        role: 'ADMIN'
      };
      sessionStorage.setItem('vnr_user', JSON.stringify(updatedUser));
      // Re-trigger login to sync UI state
      login(updated.adminUsername, updated.adminPassword).catch(() => {});
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Single Central Administrator ({settings.adminName})
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          {!isAdminLoggedIn ? (
            ...
            <form onSubmit={handleAdminLogin} className="space-y-4">
              <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 text-xs text-blue-900 dark:text-blue-200">
                <div className="font-bold flex items-center gap-1.5 mb-1">
                  <Lock className="w-4 h-4 text-blue-600" />
                  <span>Admin Authentication</span>
                </div>
                <p>Please enter your administrator password to edit username, password, display name, and CR attendance permissions.</p>
              </div>
            ...
            /* Admin Permissions & Profile Editing Panel */
            <div className="space-y-5">
              ...
              {saveSuccess && (
                <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 text-emerald-800 dark:text-emerald-200 text-xs font-bold flex items-center gap-2">
                  ...
                  <span>Admin Profile & Permissions saved successfully!</span>
                </div>
              )}

              {/* 1. Admin Profile & Credentials Section */}
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-750 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. C.Rithvik"
                      className="w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:outline-hidden font-semibold"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:outline-hidden font-mono"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. CSE"
                      className="w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:outline-hidden"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:outline-hidden font-mono font-bold"
                    />
                  </div>
                </div>
              </div>

              {/* 2. Attendance Recording Access Control */}
              <div className="space-y-2.5">
                <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                  CR Attendance Access Control
                </h4>
                ...
                <div className="space-y-2">
                  <label className="flex items-start gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-850 cursor-pointer transition-colors">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500 dark:text-slate-400">
                        CRs can record and save attendance directly without entering a passcode.
                      </div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-850 cursor-pointer transition-colors">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500 dark:text-slate-400">
                        CRs must enter the Admin Access Passcode to save attendance.
                      </div>
                    </div>
                  </label>
                </div>
              ...
              {/* 3. CR Access Passcode */}
              <div className="space-y-1.5">
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                  CR Attendance Passcode (Give this to CRs)
                </label>
              ...
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-md transition-all"
              >
                <Save className="w-4 h-4" />
                <span>Save All Changes (Profile, Password & Passcode)</span>
              </button>
            </div>
          )}
        </div>
